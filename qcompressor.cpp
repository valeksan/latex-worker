...
bool QCompressor::gzipCompress(QByteArray input, QByteArray &output, int level)
{
	...
	if(input.length())
	{
		...
		do {
			...
			strm.next_in = reinterpret_cast<unsigned char* >(input_data);
			...
			do {
				...
				int have = static_cast<int>((GZIP_CHUNK_SIZE - strm.avail_out));
...
bool QCompressor::gzipDecompress(QByteArray input, QByteArray &output)
{
	...
	if(input.length() > 0)
	{
		...
		do {
			...
			strm.next_in = reinterpret_cast<unsigned char*>(input_data);
			strm.avail_in = static_cast<unsigned int>(chunk_size);
